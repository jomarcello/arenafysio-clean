...
export default function ConversationalVoiceDemo() {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      // Connection to ElevenLabs voice agent using practice config
      const elevenlabsApiKey = process.env.NEXT_PUBLIC_ELEVENLABS_API_KEY || '';
      const agentId = practiceConfig.agentId;
      ...
      wsRef.current.onopen = () => {
        console.log('Connected to ElevenLabs Thai Voice Agent');
...
ให้คำตอบสั้นๆ กระชับ และช่วยเหลือผู้ป่วยอย่างเต็มที่`
        ...
        console.log('🏥 Thai BeautyMed configuration sent to ElevenLabs');
      ...
      wsRef.current.onmessage = (event) => {
        ...
        console.log('📨 Received ElevenLabs response:', data);
      ...
      wsRef.current.onerror = (error) => {
        ...
        setError('ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ ElevenLabs ได้');
        setConnectionStatus('disconnected');
        setIsConnected(false);
      };
  ...
  const startAudioStreaming = useCallback(async () => {
    ...
    try {
      ...
      processorRef.current.onaudioprocess = (event) => {
        ...
        // Send continuous PCM data to ElevenLabs only when listening
        try {
          const audioMessage = {
            type: 'audio_stream',
          ...
          console.log(`🎤 Sent audio chunk to ElevenLabs: ${audioMessage.audio_data.length} chars`);
        } catch (error) {
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-gray-600">
          Real-time conversational AI with ElevenLabs Thai Voice Agent
        </p>
      ...
      {isConnected && (
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            ...
            Robin (ElevenLabs Thai Voice Agent):
          </h3>
